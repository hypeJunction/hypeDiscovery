...
function get_share_action_url($provider, $guid = 0, $referrer = '', $share_url = '')
{
    ...
    return elgg_http_add_url_query_elements($base_url, array('provider' => $provider, 'guid' => $guid, 'referrer' => $referrer, 'share_url' => $share_url));
}
/**
...
function get_provider_url($provider, $entity = null, $referrer = '', $share_url = '')
{
    ...
    switch ($provider) {
        case 'facebook':
            ...
            $elements = array('u' => $share_url, 't' => $title);
            break;
        case 'twitter':
            $base_url = "https://twitter.com/intent/tweet";
            $elements = array('url' => $share_url, 'hashtags' => is_array($tags) ? implode(',', $tags) : $tags, 'via' => $via ? str_replace('@', '', $via) : '');
            break;
        case 'linkedin':
            $base_url = "http://www.linkedin.com/shareArticle";
            $elements = array('mini' => true, 'url' => $share_url, 'title' => $title, 'source' => $site->og_site_name, 'summary' => $description);
            break;
        case 'pinterest':
            $base_url = "https://pinterest.com/pin/create/button/?url";
            $elements = array('url' => $share_url, 'media' => $media, 'description' => $title);
            break;
        case 'googleplus':
            $base_url = 'https://plus.google.com/share';
            $elements = array('url' => $share_url, 'title' => $title, 'summary' => $description);
            break;
    }
    if ($base_url) {
...
function get_entity_permalink($entity, $viewtype = 'default')
{
    ...
    if (!$url || elgg_get_config('walled_garden')) {
        ...
        $segments = array('permalink', $viewtype, $entity->guid, $title);
...
function get_user_from_hash($hash = '')
{
    ...
    $users = elgg_get_entities(array('types' => 'user', 'metadata_names' => ['discovery_permanent_hash', 'discovery_temporary_hash'], 'metadata_values' => $hash, 'limit' => 1));
